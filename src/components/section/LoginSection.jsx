...
const LoginSection = () => {
  ...
  return (
    ...
    <div className="text-sm ">
      <button onClick={toggleMenu} className="group space-x-1 flex items-center cursor-grabbing">
      ...
      {isMenuOpen && (
        <div className="absolute top-16 -ml-24 my-2 bg-white rounded border p-4">
          <ul className="list-none">
            <Link to="/login">
              <li className='cursor-pointer'>
                <button className=' w-36 h-10 rounded-md bg-primaryColor text-white'>Giriş Yap</button>
              </li>
            </Link>
            <Link to="/signin">
              <li>
                <button className='w-36 h-10 rounded-md border hover:text-primaryColor border-borderColor'>Üye Ol</button>
              </li>
            </Link>
          </ul>
        </div>
